...
> SuperAgent LiquidityIQ is a multi-provider liquidity and anomaly decision-support system for mobile financial services operations. It helps super agents, field officers, and operations teams understand liquidity pressure, unusual transaction behavior, and who should coordinate the response — without claiming fraud, merging wallets, or executing financial actions.

## ✅ What this project does

- Simulates realistic MFS activity across bKash, Nagad, and Rocket
- Detects forward-looking liquidity shortages and operational anomalies
- Explains alerts with evidence, confidence, and safe human-review language
- Routes findings into a case workflow with ownership, escalation, and audit trail
- Presents everything in a working dashboard with a live FastAPI backend
- Persists simulation and case data in SQLite for repeatable demo sessions
...
2. Explore liquidity projections and alert evidence.
3. Open the case board to see assignment and escalation workflow.
4. Use the chaos toggle to show how degraded provider data lowers confidence safely.
...
# 4. Start the backend (generates synthetic data and initializes the SQLite DB)
python -m uvicorn backend.main:app --host 127.0.0.1 --port 8000
...
If pytest is not available yet, install it with:

```bash
python -m pip install pytest
```

### Run individual segment checks
...
_This is a 24-hour hackathon prototype. It does not connect to real wallets, real customer accounts, or any production financial infrastructure. It has not received regulatory review or approval. All data is synthetic. All risk signals are advisory and require human review before any real-world action._
